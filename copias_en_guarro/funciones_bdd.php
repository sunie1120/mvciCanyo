...
function crear_pestanas(){
    echo("<ul>");
	$contador=0;
	//recogemos los nombres de los servicios en un array
	while($nombreA=mysql_fetch_array($resultado_nombre)){
	$contador++;

		echo('<li><a href="#a'.$contador.'">');
		echo($nombreA[0]);//campo 0 corresponde a la consulta del nom_servei
		echo("</a></li>");
	}
	echo("</ul>");
	$contador=0;
	while($nombreB=mysql_fetch_array($resultado_desc)){
	$contador++;
	echo('<div id="a'.$contador.'">');
	$imagen=($nombreB[0]);//campo 1 corresponde a la consulta del descripcio y campo 0 a la imagen
